...
def point_cloud(experiment: int) -> None:
    ...
    spiral_workspace_folder = os.path.join(
        settings["workspace folder"], f"spiral_exp_{experiment}_{day}_{month}_{hour}_{minute}"
    )
    ...
    for route, object_key, count_group in zip(route_by_group, spiral_route_key, range(len(route_by_group))):
        ...
        workspace_folder = os.path.join(
            settings["workspace folder"], f"exp_{experiment}_{day}_{month}_{hour}_{minute}_group_{object_key}"
        )
        ...
        op_workspace_folder = os.path.join(
            settings["workspace folder"], f"op_exp_{experiment}_{day}_{month}_{hour}_{minute}_group_{object_key}"
        )

        op_image_directory_name = (
        ...
        random_workspace_folder = os.path.join(
            settings["workspace folder"], f"random_exp_{experiment}_{day}_{month}_{hour}_{minute}_group_{object_key}"
        )
