...
def get_csv_data_build(csv_path, basename):
    ...
    reward_west = list(map(float, own_west["route_reward"].values))
    reward_chinese = list(map(float, own_chinese["route_reward"].values))
    reward_old = list(map(float, own_old["route_reward"].values))

    route_reward = np.mean(reward_west + reward_chinese + reward_old)
    own_west["reward"] = route_reward
    own_chinese["reward"] = route_reward
    own_old["reward"] = route_reward
...
def plot_images_build(west_data, chinese_data, old_data):
    ...
    west_mean = west_data.groupby("reward", as_index=False)["rmse"].mean()
    chinese_mean = chinese_data.groupby("reward", as_index=False)["rmse"].mean()
    old_mean = old_data.groupby("reward", as_index=False)["rmse"].mean()

    # Criar a figura
    plt.figure(figsize=(7, 14 / 3))

    # Adicionar cada conjunto de dados como uma linha
    sns.lineplot(data=chinese_mean, x="reward", y="rmse", label=r"$O_1$", linewidth=2, marker="s")
    sns.lineplot(data=west_mean, x="reward", y="rmse", label=r"$O_2$", linewidth=2, marker="o")
    sns.lineplot(data=old_mean, x="reward", y="rmse", label=r"$O_3$", linewidth=2, marker="d")

    # Configuração dos rótulos e título
    plt.xlabel("Route Reward", fontsize=14)
    ...
    # Salvar figura
    plt.savefig(f"expr_images/rmse_x_reward.png", dpi=600)
